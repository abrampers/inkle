...
func TestDecodeLayers(t *testing.T) {
	...
	for _, test := range tests {
		...
		if test.want == true && err != nil {
			t.Errorf("DecodeFromBytes(%q) returns err = %v, where shouldn't be no error", string(test.input), err)
		} else if test.want == false && err == nil {
			t.Errorf("DecodeFromBytes(%q) returs no err, where there should be error", string(test.input))
		} else if test.nstream != len(h2.Frames()) {
			t.Errorf("DecodeFromBytes(%q) produces %d where it should be %d stream(s)", string(test.input), len(h2.Frames()), test.nstream)
		}
	}
}
